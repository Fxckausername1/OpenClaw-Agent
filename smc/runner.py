...
class PaperRunner:
    ...
    def _on_reconcile(self, event: Event) -> None:
        ...
        self.readiness.set("reconciled", ok, "" if ok else "reconciliation failed")
    ...
    def health(self) -> dict:
        return {
            ...
            "theta_stream": self.theta_stream.health() if self.theta_stream else None,
            "trade_updates": self.trade_updates.health() if self.trade_updates else None,
            "notifier": self.notifier.health() if self.notifier else None,
        }
