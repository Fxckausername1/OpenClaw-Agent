...
class TradeResolutionConfig:
    min_r_multiple: float = 0.25       # credit-spread success threshold (debits ~1.0)
...
def update_thompson_sampling(r_multiple, alpha, beta, cfg=CFG):
    """Discounted Thompson Sampling: decay then Bernoulli(R>=R_min). (spec §5.4)"""
...
# ----------------------------------------------------------------- live reconcile (cron)
def reconcile(conn):
    """Cron: poll Alpaca, transition PENDING->OPEN on entry fill, OPEN->CLOSED when the position is
    flat, resolve R-multiple. Live broker call (no-op-safe if creds/endpoint unavailable).
    NOTE: exit/close orders are issued by the orchestrator/PM; this only RECONCILES broker truth."""
    import requests
    ...
    openrows = conn.execute("SELECT trade_id, status FROM trades_ledger "
                            "WHERE status IN ('PENDING','OPEN','PARTIAL_CLOSE')").fetchall()
    if not openrows:
        return {"checked": 0, "resolved": 0}
    try:
        orders = requests.get(base + "/v2/orders", headers=h,
                              params={"status": "all", "nested": "true", "limit": 500}, timeout=25).json()
    ...
    for row in openrows:
        ...
        if not o:
            continue
        if row["status"] == "PENDING" and o.get("status") == "filled":
            conn.execute("BEGIN IMMEDIATE")
            conn.execute("UPDATE trades_ledger SET status='OPEN' WHERE trade_id=?", (row["trade_id"],))
            conn.execute("COMMIT")
        # close detection + realized PnL is wired to the orchestrator's exit policy (next phase):
        # when the closing mleg fills, the orchestrator passes realized_pnl -> resolve_trade().
    return {"checked": len(openrows), "resolved": resolved}
...
def reconcile_expirations(conn, today=None, cfg=CFG, arm=False):
    """EOD gateway: OTM expirations fire NO order-fill event, so resolve them from
    /v2/account/activities (OPEXP/OPASN/OPXRC). OPEXP credit spread -> retained premium = max profit
    -> resolve + Thompson update. OPASN -> assignment: penalize (≈max loss) + FLAG for the liquidation-
    recovery routine (logged loudly; the full auto-liquidation sequence is a separate safety module)."""
    import requests
    ...
    for r in rows:
        ...
        if exps and min(exps) <= today:
            stale.append((r["trade_id"], meta))
    if not stale:
        return {"stale": 0, "resolved": 0, "assigned": 0}
    try:
        KEY = (ROOT / "credentials" / "alpaca_key.txt").read_text().strip()
        SEC = (ROOT / "credentials" / "alpaca_secret.txt").read_text().strip()
        h = {"APCA-API-KEY-ID": KEY, "APCA-API-SECRET-KEY": SEC}
        acts = requests.get("https://paper-api.alpaca.markets/v2/account/activities", headers=h,
                            params={"activity_types": "OPEXP,OPASN,OPXRC"}, timeout=25).json()
    except Exception as e:
    ...
    for tid, meta in stale:
        ...
        risk = float(conn.execute("SELECT initial_risk FROM trades_ledger WHERE trade_id=?",
                                  (tid,)).fetchone()["initial_risk"])
        if "OPASN" in types:                            # CATASTROPHIC: assignment -> auto-liquidation
            opasn_liquidate(conn, tid, meta, arm=arm, cfg=cfg)
            assigned += 1
        elif "OPEXP" in types or not types:             # both legs worthless OTM at expiry
            # A CREDIT spread keeps the full premium (max PROFIT, +); a DEBIT spread loses the full
            # debit paid (max LOSS, -). entry_credit stores the per-share magnitude for BOTH
            # structures, so the sign must come from `structure` -- without this a debit spread that
            # expired worthless booked a positive max-profit instead of its max loss (latent sign bug,
            # same family as the exec_px multi-leg sign bug, found 2026-06-29).
...
def main():
    ...
    if a.reconcile:
        print("reconcile:", reconcile(conn))
    if a.reconcile_exp:
