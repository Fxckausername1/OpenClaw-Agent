...
MAX_PRICE = 250.0
# 12 checks was calibrated assuming the "full */15 scan behavior" advance_watch()'s own
# docstring describes (12 x 15min = 3h intended watch window, matching what was backtested).
# FIXED 2026-07-19 (audit #9b): the full-scan cron (mean_reversion_wrapper.sh) actually runs
# */5, not */15 -- confirmed live via crontab -- and `checks` only increments on full-scan
# passes (advance_watch's manage_lifecycle=not watch_only; the separate */1 watch-only loop
# deliberately never touches checks/expiry, "the */15 scan stays the single owner of a
# setup's lifecycle" per its own docstring). At the real */5 cadence, 12 checks silently
# expired watches after ~60min instead of the intended ~3h. Raised to 36 (36x5min=3h) to
# restore the originally-intended/backtested window WITHOUT touching the cron schedule --
# smaller footprint than slowing the full scan, which would also delay how fast brand-new
# setups get first noticed (the */1 watch-only loop still handles fast trigger detection
# for names already being watched, unaffected either way).
MAX_WATCH_CHECKS = 36
...
def get_5m_data(ticker, days=5):
    ...
    if cached is not None and len(cached) >= 25:
        _LAST_DATA_SOURCE[ticker] = "alpaca_iex_batch"
        return cached
    if ticker in _BATCH_REQUESTED:
        _LAST_DATA_SOURCE[ticker] = "alpaca_iex_batch_missing"
        return None
    df = fetch_5m_alpaca(ticker, days)
    if df is not None and len(df) >= 25:
        _LAST_DATA_SOURCE[ticker] = "alpaca_iex_symbol"
        return df
    y = fetch_5m_yfinance(ticker, days)
    _LAST_DATA_SOURCE[ticker] = "yfinance" if y is not None else "unavailable"
    return y if y is not None else df


def data_source_for(ticker):
...
def compute(ticker, today_date):
    ...
    if df is None or len(df) < 25:
        return None
    # CLOSED BARS ONLY (2026-07-19, audit #9a): a bar covering [t, t+5min) isn't done until
    # t+5min -- using it before then means every downstream calc (SMA/STD/RSI/z-score/VWAP)
    # sees partial price/volume that can still change. ORB already excludes its still-forming
    # bar for exactly this reason (orb_scanner.py's `post = post[post.index <= now - 5min]`,
    # "partial volume that understates relvol"); MR's compute() never had the equivalent
    # guard, so a live MR read could act on data a replay/backtest could never have seen
    # (historical data is always final). Drop the bar here if it hasn't closed yet.
    now_et = datetime.now(ET)
    if df.index[-1] + timedelta(minutes=5) > now_et:
        df = df.iloc[:-1]
        if len(df) < 25:
            return None
    close = df["Close"]
    ...
    today_mask = np.array(df.index.date == today_date)
    today_rows = df[today_mask]
    prior_rows = df[~today_mask]
    overnight_gap_pct = None
    if len(today_rows) and len(prior_rows) and float(prior_rows["Close"].iloc[-1]) != 0:
        overnight_gap_pct = ((float(today_rows["Open"].iloc[0]) /
                              float(prior_rows["Close"].iloc[-1])) - 1.0) * 100.0
    z_dates = set(df.index[-20:].date)
    ...
    return {
        ...
        "bars_into_session": int(len(today_rows)),
        "overnight_gap_pct": round(overnight_gap_pct, 4) if overnight_gap_pct is not None else None,
        "z_window_spans_gap": len(z_dates) > 1,
        "bar_time": df.index[-1].isoformat(),
        "bar_age_seconds": round(max(0.0, (now_et - df.index[-1]).total_seconds()), 2),
        "data_source": data_source_for(ticker),
    }
...
def regime_is_rotational(ticker):
    ...
    using the PRIOR completed hour (no intra-hour peeking). Returns None when data is
    insufficient: unknown observations remain in the plain cohort but are never mislabeled
    as rotational. Computed only at trigger time (rare).

    FIXED 2026-07-19 (audit Tier-2 #7): deliberately bypasses get_5m_data()/the shared
    full-scan batch cache and fetches directly instead. The batch cache is populated once
    per scan at whatever depth THAT scan's own needs require -- days=2 for compute()'s
    ~20-bar SMA/RSI warmup (see prefetch_batch_bars's call site) -- which get_5m_data()
    happily serves to ANY caller whose request passes a flat len(cached)>=25 BAR-count
    check, regardless of calendar-day SPAN. 2 days of 5-min bars is >150 bars (passes that
    check easily) but only ~13 hourly bars once resampled -- short of the 52 bars this
    function's own `len(h1) < REGIME_EMA_SLOW + 2` check requires below -- so every
    full-scan-triggered call silently fell open to True, while watch-loop triggers (never
    part of the batch request, so get_5m_data() already fell through to a real per-symbol
    fetch there) computed a real value. Confirmed by the codebase's own audit: this made the
    regime tag a mixture of "always True" (full-scan) and "actually computed" (watch-loop)
    rows, contaminating the live 1h EMA20/50 A/B regime experiment. Only ~1 call per actual
    trade trigger (rare), so a real per-symbol fetch here carries none of the "thousands of
    tickers per scan" slowdown risk that motivated the batch-cache-first design in the first
    place -- this is exactly what watch-loop triggers already correctly do today."""
    try:
        df = fetch_5m_alpaca(ticker, REGIME_DAYS)
        if df is None or len(df) < 25:
            y = fetch_5m_yfinance(ticker, REGIME_DAYS)
            df = y if y is not None else df
...
def run_scan(now_et, watch_only, max_tickers=100):
    ...
    trigger_alerts = []
    trigger_records = []
    scan_id = now_et.isoformat(timespec="seconds") + (":watch" if watch_only else ":full")

    def emit(event_type, payload, entity_id, suffix=""):
        if dl is not None:
            dl.emit("mean_reversion_scanner", event_type, payload, entity_id=entity_id,
                    event_time=now_et, session_date=today_date.isoformat(),
                    dedupe_key=f"{scan_id}|{event_type}|{entity_id}|{suffix}")
    ...
    def record(tk, side, entry, stop, t1, t2, rr, shares, notional, risk_dollars, regime_ok,
        ...
        rec = {
            ...
            "regime_ok": regime_ok,   # True/False/None; unknown is excluded from filtered cohort
            ...
            "bars_into_session": m.get("bars_into_session"),
            "overnight_gap_pct": m.get("overnight_gap_pct"),
            "z_window_spans_gap": m.get("z_window_spans_gap"),
        ...
        emit("mr_trigger", {**rec, "reference_price": m["close"], "track_outcome": True},
             rec["trade_id"], "trigger")
    ...
    for tk in tickers:
        if tk in blacklist:
            emit("mr_filter", {"ticker": tk, "decision": "earnings_blackout",
                               "track_outcome": False}, tk, "earnings")
            continue
        try:
            ...
            continue
        if m is None:
            emit("data_quality", {"ticker": tk, "decision": "insufficient_or_missing_bars",
                                  "track_outcome": False}, tk, "no_data")
            continue
        ...
        for side in ("SHORT", "LONG"):
            ...
            if st is None:
                ...
                setup = setup_short if side == "SHORT" else setup_long
                price_ok = m["close"] <= MR_MAX_PRICE
                decision = "price_cap" if setup and not price_ok else "no_setup"
                if side == "SHORT" and setup_short and price_ok:
                    state[key] = {"stage": "watch", "signal_low": m["low"],
                                  "extreme_high": m["high"], "checks": 0}
                    decision = "watch_created"
                    watch_alerts.append(f"{tk} SHORT-watch | px={m['close']:.2f} z={m['z']:.2f} "
                                        f"RSI={m['rsi']:.0f} {m['vwap_dev']*100:+.1f}% vs VWAP")
                elif side == "LONG" and setup_long and price_ok:
                    ...
                    watch_alerts.append(f"{tk} LONG-watch | px={m['close']:.2f} z={m['z']:.2f} "
                                        f"RSI={m['rsi']:.0f} {m['vwap_dev']*100:+.1f}% vs VWAP")
                emit("mr_evaluation", {"ticker": tk, "side": side, "decision": decision,
                                       "setup": bool(setup), "price_ok": bool(price_ok),
                                       "reference_price": m["close"], "track_outcome": True,
                                       "features": m, "thresholds": {
                                           "z": Z_THRESH, "rsi_oversold": RSI_OVERSOLD,
                                           "rsi_overbought": RSI_OVERBOUGHT,
                                           "vwap_dev": VWAP_DEV_PCT, "max_price": MR_MAX_PRICE}},
                     key, decision)
            elif st["stage"] == "watch":
                before = dict(st)
                status, vals = advance_watch(side, st, m, manage_lifecycle=not watch_only)
                emit("mr_watch_transition", {"ticker": tk, "side": side, "status": status,
                                              "state_before": before, "state_after": dict(st),
                                              "reference_price": m["close"],
                                              "track_outcome": True, "features": m},
                     key, status)
                if status == "trigger":
                    ...
                    if regime_ok is True:
                        line += "  [regime rotational]"
                    elif regime_ok is False:
                        line += "  [trending - plain cohort only]"
                    else:
                        line += "  [regime unknown - plain cohort only]"
