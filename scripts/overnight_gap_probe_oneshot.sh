#!/usr/bin/env bash
# ONE-SHOT (self-removing): holdout test of gen_overnight_gap, 6th of 8 sequential
# research jobs tonight (2026-07-08).
set -u
export HOME="/home/heff"
ROOT="/home/heff/.openclaw/workspace"
LOG="$ROOT/logs/overnight_gap_probe.log"
MARKER="overnight_gap_probe_oneshot.sh"

[ "$(date -u +%Y-%m-%d)" = "2026-07-08" ] || exit 0
exec 9>/tmp/overnight_gap_probe_oneshot.lock
flock -n 9 || exit 0
...
echo "==== overnight_gap holdout probe $(date -u) ====" >> "$LOG"
cd "$ROOT" || exit 1
nice -n 19 ionice -c3 ./venv/bin/python "$ROOT/backtest_overnight_gap.py" >> "$LOG" 2>&1
RC=$?
SUMMARY=$(grep -A1 "^  overnight_gap:" "$LOG" | tail -20)
openclaw message send --channel telegram --target 7590346809 \
  --message "overnight_gap holdout probe done (rc=$RC):
