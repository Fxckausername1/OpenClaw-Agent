#!/usr/bin/env bash
# ONE-SHOT (self-removing): holdout test of gen_vol_reversion, 5th of 8 sequential
# research jobs tonight (2026-07-08).
set -u
export HOME="/home/heff"
ROOT="/home/heff/.openclaw/workspace"
LOG="$ROOT/logs/vol_reversion_probe.log"
MARKER="vol_reversion_probe_oneshot.sh"

[ "$(date -u +%Y-%m-%d)" = "2026-07-08" ] || exit 0
exec 9>/tmp/vol_reversion_probe_oneshot.lock
flock -n 9 || exit 0
...
echo "==== vol_reversion holdout probe $(date -u) ====" >> "$LOG"
cd "$ROOT" || exit 1
nice -n 19 ionice -c3 ./venv/bin/python "$ROOT/backtest_vol_reversion.py" >> "$LOG" 2>&1
RC=$?
SUMMARY=$(grep -A1 "^  vol_reversion:" "$LOG" | tail -20)
openclaw message send --channel telegram --target 7590346809 \
  --message "vol_reversion holdout probe done (rc=$RC):
