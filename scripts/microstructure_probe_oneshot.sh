#!/usr/bin/env bash
# ONE-SHOT (self-removing): holdout test of gen_microstructure, 7th of 8 sequential
# research jobs tonight (2026-07-08).
set -u
export HOME="/home/heff"
ROOT="/home/heff/.openclaw/workspace"
LOG="$ROOT/logs/microstructure_probe.log"
MARKER="microstructure_probe_oneshot.sh"

[ "$(date -u +%Y-%m-%d)" = "2026-07-08" ] || exit 0
exec 9>/tmp/microstructure_probe_oneshot.lock
flock -n 9 || exit 0
...
echo "==== microstructure holdout probe $(date -u) ====" >> "$LOG"
cd "$ROOT" || exit 1
nice -n 19 ionice -c3 ./venv/bin/python "$ROOT/backtest_microstructure.py" >> "$LOG" 2>&1
RC=$?
SUMMARY=$(grep -A1 "^  microstructure:" "$LOG" | tail -20)
openclaw message send --channel telegram --target 7590346809 \
  --message "microstructure holdout probe done (rc=$RC):
