#!/usr/bin/env bash
# ONE-SHOT (self-removing): modern rebuild of the EOD-vs-hold-mode exit test
# (backtest_exit_holdmode.py). Last of the 2026-07-08 research chain.
set -u
export HOME="/home/heff"
ROOT="/home/heff/.openclaw/workspace"
LOG="$ROOT/logs/exit_holdmode_probe.log"
MARKER="exit_holdmode_probe_oneshot.sh"

[ "$(date -u +%Y-%m-%d)" = "2026-07-08" ] || exit 0
exec 9>/tmp/exit_holdmode_probe_oneshot.lock
flock -n 9 || exit 0
...
echo "==== exit_holdmode probe $(date -u) ====" >> "$LOG"
cd "$ROOT" || exit 1
nice -n 19 ionice -c3 ./venv/bin/python "$ROOT/backtest_exit_holdmode.py" >> "$LOG" 2>&1
RC=$?
SUMMARY=$(tail -30 "$LOG")
openclaw message send --channel telegram --target 7590346809 \
  --message "exit_holdmode probe done (rc=$RC):
