...
# Parameters — IDENTICAL to v2 (strategy logic unchanged; only the data loading
# was hardened for memory: column projection, cheap symbol scan, per-symbol release).
PORTFOLIO=800.0
PER_TRADE=266.0
MAX_OPEN=3
ENTRY_WINDOW_MIN=90
...
NEEDED_COLS=['ts_event','open','high','low','close']   # fix 1: project only used cols
...
# fix 3: cheaper symbol list via pyarrow unique() per file (no per-row-group pandas)
symbols = set()
for fn in files:
    col = pq.read_table(fn, columns=['symbol']).column('symbol')
...
print('Found symbols:', len(symbols), flush=True)
...
def process_symbol(sym):
    """Per-symbol processing in its own scope so all frames are freed on return
    (fix 2). Logic is byte-identical to v2's inner block; symbol-level `continue`
    became `return`."""
    filt = (pc.field('symbol') == sym)
    try:
        table = dataset.to_table(filter=filt, columns=NEEDED_COLS)   # fix 1
    except Exception:
        return
    if table.num_rows == 0:
        return
...
for idx, sym in enumerate(symbols):
    if (idx+1) % 50 == 0:
        print(f'Processing {idx+1}/{len(symbols)}: {sym}', flush=True)
    process_symbol(sym)
    gc.collect()   # fix 2: return the symbol's memory before the next one
...
with open(os.path.join(OUT_DIR,'ict_backtest_trades_v3.json'),'w') as f:
    json.dump(all_trades,f,indent=2)
with open(os.path.join(OUT_DIR,'ict_backtest_summary_v3.json'),'w') as f:
