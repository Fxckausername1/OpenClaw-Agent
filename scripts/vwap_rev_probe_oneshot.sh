#!/usr/bin/env bash
# ONE-SHOT (self-removing): holdout test of gen_vwap_rev, 4th of 8 sequential research
# jobs tonight (2026-07-08). Waits for all prior jobs by name before starting.
set -u
export HOME="/home/heff"
ROOT="/home/heff/.openclaw/workspace"
LOG="$ROOT/logs/vwap_rev_probe.log"
MARKER="vwap_rev_probe_oneshot.sh"

[ "$(date -u +%Y-%m-%d)" = "2026-07-08" ] || exit 0
exec 9>/tmp/vwap_rev_probe_oneshot.lock
flock -n 9 || exit 0
...
echo "==== vwap_rev holdout probe $(date -u) ====" >> "$LOG"
cd "$ROOT" || exit 1
nice -n 19 ionice -c3 ./venv/bin/python "$ROOT/backtest_vwap_rev.py" >> "$LOG" 2>&1
RC=$?
SUMMARY=$(grep -A1 "^  vwap_rev:" "$LOG" | tail -20)
openclaw message send --channel telegram --target 7590346809 \
  --message "vwap_rev holdout probe done (rc=$RC):
