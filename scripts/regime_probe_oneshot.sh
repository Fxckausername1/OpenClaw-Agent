#!/usr/bin/env bash
# ONE-SHOT (self-removing): post-close run of the regime_ok holdout probe.
# Deferred from RTH 2026-07-08 because the single-core box was busy with live
# scanners (never run 2 heavy jobs concurrently). Self-deletes its own cron line
# after a successful run, and date-guards so it only ever fires on 2026-07-08.
set -u
export HOME="/home/heff"
ROOT="/home/heff/.openclaw/workspace"
LOG="$ROOT/logs/regime_holdout_probe.log"
MARKER="regime_probe_oneshot.sh"

# date-guard: only 2026-07-08
[ "$(date -u +%Y-%m-%d)" = "2026-07-08" ] || exit 0

exec 9>/tmp/regime_probe_oneshot.lock
flock -n 9 || exit 0
...
# wait for a clear (no live heavy job) window, up to ~5 min
...
echo "==== regime holdout probe $(date -u) ====" >> "$LOG"
cd "$ROOT" || exit 1
nice -n 19 ionice -c3 ./venv/bin/python /tmp/regime_holdout_probe.py >> "$LOG" 2>&1
RC=$?

# compact telegram: the per_trade lines + headers
SUMMARY=$(grep -E "=====|baseline|regime_ok|date split" "$LOG" | tail -20)
openclaw message send --channel telegram --target 7590346809 \
  --message "Regime_ok holdout probe done (rc=$RC):
$SUMMARY" >/dev/null 2>&1

# self-remove this cron line so it never repeats
