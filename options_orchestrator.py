...
def process_exits(arm=False, tp_frac=TP_FRAC, sl_mult=SL_MULT):
    ...
    for row in rows:
        ...
        if meta.get("structure") == "single_leg_long":
            # SMC_TRIANGLE (live_heff_smc_executor.py) and any other single-leg long position
            # shares this table but has its own dedicated exit manager
            # (live_heff_smc_exit_manager.py) with its own validated target/stop/time-stop rule.
            # This sweep's TP/SL math is credit/debit-SPREAD-specific (spread_close_cost walks a
            # `legs` list expecting SELL+BUY pairs, and the block below reads meta["entry_credit"],
            # a key single-leg positions never set) -- skip rather than crash or double-manage.
            continue
